                        ...
                        <div class="inner">
                            <section id="woocommerce_product_categories-2"
                                ...
                                <ul class="product-categories children">
                                    @foreach($show_loaisanpham as $item_show_loaisanpham)
                                    @if($item_show_loaisanpham->parent_id == null)
                                    <li class="cat-item cat-item-111 " data-id="2">
                                        <p>{{$item_show_loaisanpham->name}}</p>

                                        <ul class="children">
                                            @foreach($show_loaisanpham as $vt_category_child)
                                            @if($vt_category_child->parent_id == $item_show_loaisanpham->id)
                        ...
                        <main id="main" class="site-main" role="main">
                            ...
                            <div class="osf-sorting-wrapper">
                                <div class="osf-sorting">
                                    <div class="osf-sorting-group col-lg-6 col-sm-12"><a href=""
                                            style="font-size: 20px;"><i class="fas fa-bars" style="color: #80b500;"></i>
                                            Tất cả</a>
                                    </div>
                            ...
                            <div class="columns-3">
                                <ul class=" products columns-3 ajax-showdata ">
                                    @foreach($show_sanpham as $item_show_sanpham)
                                    <li
                                        class="show_data_pro list_product product type-product post-2840 status-publish first instock product_cat-cough-cold-flu has-post-thumbnail shipping-taxable purchasable product-type-simple">
                                        <div class="product-block" style="padding: 15px;">
                                            <div class="product-transition">
                                                <div class="product-image">
                                                    <img width="400" height="534"
                                                        src="{{asset('storage/'.$item_show_sanpham->image)}}"
                                                        class="attachment-shop_catalog size-shop_catalog wp-post-image"
                                                        alt="">
                                                </div>
                                                <a href="{{route('chitietsanpham',$item_show_sanpham->slug)}}"
                                                    class="woocommerce-LoopProduct-link woocommerce-loop-product__link"></a>
                                            </div>
                                            <h3 class="woocommerce-loop-product__title"><a
                                                    href="{{route('chitietsanpham',$item_show_sanpham->slug)}}">{{$item_show_sanpham->seo_title}}</a>
                                            </h3>

                                            <!-- <div class="woocommerce-product-details__short-description"
                                                style="font-size: 12px;line-height: 15px;">
                                                <p style="color: #f6b822;">tirte</p>
                                            </div> -->
                                            <div class="woocommerce-product-details__short-description"
                                                style="font-size: 12px;">{!!$item_show_sanpham->excerpt!!}</div>
                                                ...
                                                <div class="group-label">
                                                    <span class="price">
                                                        <span class="woocommerce-Price-amount amount"
                                                            ...
                                                            {{number_format($item_show_sanpham->price)}} VND </span>
                                                    </span>
                                                </div>
                                                <div class="opal-add-to-cart-button tooltipstered">
                                                    <a href="{{asset('/cart/add/'.$item_show_sanpham->id)}}"
                                                        data-quantity="1"
                                                        class="button product_type_simple add_to_cart_button ajax_add_to_cart"
                                                        data-product_id="2840" data-product_sku=""
                                                        style="font-size: 18px; text-decoration: none;"
                                                        aria-label="Add “Abreva Cold Sore Fever Blister Treatment Cream” to your cart"
                                                        rel="nofollow">Thêm vào giỏ hàng &emsp;&emsp;<i
                        ...
                        element += '<li class="show_data_pro list_product product type-product post-2840 status-publish first instock product_cat-cough-cold-flu has-post-thumbnail shipping-taxable purchasable product-type-simple">' +
                                        '<div class="product-block" style="padding: 15px;">' +
                                            '<div class="product-transition">' +
                                                ...
                                                '</div>'+
                                                '<a href="{{ route('chitietsanpham','') }}/'+ slug +'" class="woocommerce-LoopProduct-link woocommerce-loop-product__link">'+
                                                '</a>'+
                                            '</div>' +
                                            '<h3 class="woocommerce-loop-product__title">'+
                                                '<a href="{{ route('chitietsanpham','') }}/'+ slug +'">'+ res[i].seo_title +
                                                '</a>'+
                                            '</h3>'+
                                            '<div class="woocommerce-product-details__short-description" style="font-size: 12px;line-height: 15px;">'+
                                                '<p style="color: #f6b822;">'+ res[i].excerpt +
                                                '</p>'+
                                            '</div>'+
                                            ...
                                            '<div class="caption">'+
                                                ...
                                                '</div>'+
                                                '<div class="opal-add-to-cart-button tooltipstered">'+
                                                    '<a data-quantity="1" class="button product_type_simple add_to_cart_button ajax_add_to_cart" data-product_id="2840" data-product_sku="" style="font-size: 18px; text-decoration: none;"  rel="nofollow" href="{{ URL('/cart/add/','') }}/'+ res[i].id +'" >'+ 'Thêm vào giỏ hàng' + '&emsp;&emsp;<i class="fas fa-shopping-basket"></i>'+
                                                    '</a>'
                                                '</div>'+
